...
namespace Office365PlannerTask.Models
{
    public class GroupRespository
    {
        ...
        public async Task<List<MyPlan>> GetGroupPlans(string groupId)
        {
            var graphServiceClient = await GetGraphServiceAsync();
            var request = await graphServiceClient.Groups[groupId].Planner.Plans.Request().GetAsync();
            var retPlans = request.CurrentPage.Select(x => new MyPlan
            {
                id = x.Id,
                title = x.Title,
                owner = x.Owner,
                createdBy = x.CreatedBy.User.DisplayName,
                taskCount = 0
            }).ToList();
